...
import { Row, Col, Container, Button, ToggleButton, ToggleButtonGroup } from 'react-bootstrap'
import './styles.css'
import { Sensor } from '../'
...
export const SensorMap = ({ workingSensors, faultySensors, maintenanceSensors }) => {
  ...
  let allfloors = [...workingSensors, ...faultySensors, ...maintenanceSensors].map(sensor => sensor.floor).sort((a, b) => Math.sign(a - b))
  ...
  workingSensors = workingSensors.filter(sensor => sensor.floor === displayFloor)
  faultySensors = faultySensors.filter(sensor => sensor.floor === displayFloor)
  maintenanceSensors = maintenanceSensors.filter(sensor => sensor.floor === displayFloor)
    ...
    <>
      <Container fluid>
        ...
        <Row className='mt-n4 pb-3' >
          ...
          <Col>
            <ToggleButtonGroup className="h-100" value={displayFloor} onChange={setDisplayFloor} name="floorSelectionGroup">
              {[...floors].map(floor => (
                <ToggleButton value={floor} variant="outline-primary" className="d-flex align-items-center" key={floor}> {floor} </ToggleButton>
              ))}
            </ToggleButtonGroup>
          </Col>
        </Row>
        ...
        <Row className='justify-content-center'>
          ...
          <Col className='p-0' >
            ...
            <img className='w-100 floor-map' id="wp" src={floorConfig.map} alt='Map of the floor' />
          </Col>
          <Col xs={0} lg={1}></Col>
